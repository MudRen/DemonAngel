...
{
	object owner;
	mapping armor_prop, applied_prop, add_attr;
	string *apply, type;
	...
	// 额外附加值
	if( query("add_attr") && mapp(add_attr = query("add_attr")) ) {
		apply = keys(add_attr);
		applied_prop = owner->query_temp("apply");
		if( !mapp(applied_prop) ) applied_prop = ([]);
		for(int i = 0; i<sizeof(apply); i++)
			if( undefinedp(applied_prop[apply[i]]) )
				applied_prop[apply[i]] = add_attr[apply[i]];
			else
				applied_prop[apply[i]] += add_attr[apply[i]];
	}
...
int wield()
{
	...
	if( !wizardp(owner) && query("skill")>20
	&& query("skill")>owner->query_wp_sk(query("skill_type")) )
		return notify_fail("你的技能不足于使用这把武器。\n");
	...
	if( query("add_attr") && mapp(add_attr = query("add_attr")) ) {
		apply = keys(add_attr);
		for(int i = 0; i<sizeof(apply); i++)
			owner->add_temp("apply/" + apply[i], add_attr[apply[i]]);
	}

	if( query_temp("spell_effect/coating") ) {
		set_temp("coating_ob", owner);
		owner->add_temp("apply/str", 1);
		owner->add_temp("apply/damage", 10);
	}

	if(owner->query("invoke_times")) set_temp("invoke_times", owner->query("invoke_times"));
	if(owner->query("broken")) set_temp("broken", owner->query("broken"));
...
{
	object owner;
	mapping prop, applied_prop, add_attr;
	...
	if( query_temp("spell_effect/coating") ) {
		set_temp("coating_ob", 0);
		owner->add_temp("apply/str", -1);
		owner->add_temp("apply/damage", -10);
	}

	if(query_temp("invoke_times")) owner->set("invoke_times", query("invoke_times") );
	if(query_temp("broken")) owner->set("broken", query("broken") );
