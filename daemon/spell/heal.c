...
int cast_spell(object me, object target)
{
	...
	call_out("heal", 0, me, target);
	return 1;
}

int heal(object me, object target)
{
	if( random( 40 ) > me->query_skill("cast",1)*2/3 + me->query_kar()*4 ) {
		...
		target->receive_heal("hp", 18+random(2) );
		message_vision(HIY"一道圣光笼罩着$N, $N身上的伤口奇迹似的愈合了...\n"NOR, target);
	}
	return 1;
}
