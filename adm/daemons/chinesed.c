...
void add_kingdom_name( string id, string name )
{
	if(!kingdom) kingdom = ([]);
	kingdom[id] = name;
	save();
...
void remove_kingdom_name( string id )
{
	map_delete( kingdom, id );
	save();
}

string chinese_kingdom(string id)
{
	if( !undefinedp( kingdom[id] ) ) return kingdom[id];
	else return id;
}
