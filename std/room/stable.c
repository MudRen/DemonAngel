...
int do_list(string arg)
{
	...
	if( !env->query("data") || !arrayp(env->query("data")) )
		return notify_fail("目前马房空荡荡，并无任何马匹驻足于此。\n");
...
int do_garrison(string arg)
{
	...
	if( !arg ) return notify_fail("你要招唤哪一只马匹回营？\n");
	if( !(ob = present(arg, env))) return notify_fail("没有这个生物 !!\n");
	if( ob->query("level") > 30 ) return notify_fail("马房的生物等级最大只能到 30。\n");
	if( ob->query("type") != "beast" ) return notify_fail("马房内只能野兽才能驻足。\n");
	if(  env->query("data") && arrayp(list = env->query("data")) && sizeof(list) >= 30 )
		return notify_fail("这里的马房太拥挤了，不能再驻足马匹。\n");

	if( member_array(base_name(ob), list) != -1 ) {
		write(ob->name_id()+"回营驻足了。\n");
	...
	if( flag ==1 ) {
		...
		write(ob->name_id()+"的资料有问题, 无法搬到这里驻足!!\n");
	return 1;
}
...
int do_take(string arg)
{
	...
	if( !arg ) return notify_fail("你要招唤那一只马匹 ?\n");
	num = atoi(arg);
	if( num == -1 ) return notify_fail("你要招唤那一只马匹 ?\n");

	if( !env->query("data") || !arrayp(env->query("data")) )
		return notify_fail("这里没有这只马匹。\n");
	if( zerop(list = env->query("data")) ) return notify_fail("这里没有这只马匹。\n");
	if( num > (sizeof(list)-1) || num < 0 ) return notify_fail("这里没有这只马匹。\n");
	if( find_object(list[num]) ) return notify_fail("这只马匹已经被带出营区了。\n");
