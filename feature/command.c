...
/*		case "(arch)":
	...
	if( this_object()->query("kingdom/id") && !check_kingdom(this_object()->query("kingdom/id")) ) {
		this_object()->delete("kingdom");
		this_object()->delete("home");
		this_object()->delete("cwd");
		this_object()->delete("title");
	}

	if( this_object()->query("kingdom/king") ) add_path(KIN_PATH);
	else if( this_object()->query("kingdom/minister") ) add_path(MIN_PATH);
}
