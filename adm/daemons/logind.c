...
//	user->set("potential", 99);
	...
	user->set("max_lv", 10);		// 限制冒险者的最高等级

	user->set("new_char", 1);		// 新手标记
...
//	cloth->wear();
	...
	CHANNEL_D->do_channel( this_object(), "sys",
		sprintf("%s 由%s连线进入。", user->query("id"), query_ip_name(user)) );
...
varargs void reconnect(object ob, object user, int silent)
{
	...
	CHANNEL_D->do_channel( this_object(), "sys",
		sprintf("%s 由%s重新连线进入。", user->query("id"), query_ip_name(user)) );
	UPDATE_D->check_user(user);
}
